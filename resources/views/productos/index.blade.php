<x-layouts::app :title="__('Productos')">
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <flux:button class="cursor-pointer" icon="plus" variant="primary" href="{{ route('productos.create') }}">{{ __('Agregar Producto') }}</flux:button>
        </div>
    ...
    <script>
        document.addEventListener('livewire:navigated', () => {
            const divTable = document.getElementById('productos-table');
            if (!divTable) return;
            ...
            const table = new Tabulator(divTable, {
                ...
                <div class="flex flex-col items-center justify-center p-12 text-center">
                    <flux:icon name="cube" class="size-12 text-zinc-400 mb-4" />
                    <flux:heading level="3">{{ __('No hay productos') }}</flux:heading>
                    <flux:text>{{ __('Comienza agregando tu primer producto.') }}</flux:text>
                </div>
                `,
                layout: 'fitDataFill',
                columns: [{
                    ...
                    },
                    {
                        ...
                        formatter: (cell) => {
                            ...
                            const editUrl = `{{ route('productos.edit', ':id') }}`.replace(':id', id);
                            const duplicateUrl = `{{ route('productos.duplicate', ':id') }}`.replace(':id', id);
                            const destroyUrl = `{{ route('productos.destroy', ':id') }}`.replace(':id', id);
