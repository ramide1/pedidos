                ...
                <section class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <flux:card class="p-4 flex justify-between items-center group hover:border-primary transition-colors">
                            ...
                            <flux:button icon="plus" size="sm" variant="ghost" class="group-hover:bg-primary/10 cursor-pointer" onclick="addItem('{{ $producto->id }}', '{{ $producto->nombre }}', '{{ $producto->precio }}')" />
                        </flux:card>
                        @endforeach
                    </div>
                </section>
            ...
            <div class="space-y-6">
                <flux:card class="sticky top-6">
                    ...
                    <form action="{{ route('pedidos.store') }}" method="POST" id="order-form" class="space-y-4">
                        ...
                        <flux:button type="submit" variant="primary" class="w-full cursor-pointer" id="submit-btn" disabled>
                            {{ __('Realizar Pedido') }}
                        </flux:button>

                        <flux:button variant="primary" class="w-full cursor-pointer" id="wp-btn" style="--color-accent: #25D366; --color-accent-foreground: white; white-space: normal; min-height: 2.5rem;" disabled>
    ...
    <script>
                    ...
                    div.innerHTML = `
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex gap-1">
                                    <button type="button" class="p-1 hover:bg-zinc-100 rounded cursor-pointer" onclick="subtractItem('${item.id}')">-</button>
                                    <button type="button" class="p-1 hover:bg-zinc-100 rounded cursor-pointer" onclick="addItem('${item.id}', '${safeName}', '${item.precio}')">+</button>
                                </div>
                            </div>
        ...
        const generateWhatsAppMessage = () => {
            ...
            cart.forEach((item) => {
                ...
                message += `   Precio unitario: $${item.precio.toFixed(2)}%0A%0A`;
            });
